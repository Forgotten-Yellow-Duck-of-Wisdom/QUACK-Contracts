...
contract AdminFacet is AccessControl {
     ...
     * @param _bodyColorIds The item ID representing the body color applied to NFTs in the cycle.
     */
    event CreateCycle(uint256 indexed _cycleId, uint256 _cycleMaxSize, uint256 _eggsPrice, uint256[] _bodyColorIds);
    /**
     ...
     * @param _bodyColorIds The item ID representing the body color applied to NFTs in the new cycle.
     ...
     */
    function createCycle(uint24 _cycleMaxSize, uint256 _eggsPrice, uint256[] calldata _bodyColorIds)
        external
        isAdmin
        returns (uint16 cycleId_)
    {
        ...
        for (uint256 i; i < _bodyColorIds.length; i++) {
            s.cycles[cycleId_].allowedBodyColorIds.push(_bodyColorIds[i]);
        }
        emit CreateCycle(cycleId_, _cycleMaxSize, _eggsPrice, _bodyColorIds);
    }
